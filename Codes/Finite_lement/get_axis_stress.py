...
for j in range(1):
    ...
    img_path = 'BayesCNN/Aerogel_data/interpolation/anomaly_test/anomaly/image.png'
    ...
    #---------Defining Subdomains---------V = FunctionSpace(mesh, 'P', 2)-----
    ...
    sigma_xx = project(s[0,0], V)
    sigma_yy = project(s[1,1], V)
    sigma_xy = project(s[0,1], V)

    sigma_xx_file = File("sigma_xx.pvd")
    sigma_yy_file = File("sigma_yy.pvd")
    sigma_xy_file = File("sigma_xy.pvd")

    sigma_xx_file << sigma_xx
    sigma_yy_file << sigma_yy
    sigma_xy_file << sigma_xy
...
#            strain_energy_data.to_csv(f, header=f.tell()==0, index=False) 
